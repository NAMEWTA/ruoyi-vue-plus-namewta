...
import org.dromara.profile.enterprise.persistence.MybatisEnterpriseApplicationRepository;
import org.dromara.profile.enterprise.persistence.mapper.EnterpriseApplicationMapper;
import org.dromara.profile.enterprise.verification.EnterpriseManualVerificationProvider;
import org.dromara.profile.enterprise.verification.EnterpriseVerificationAttemptCoordinator;
import org.dromara.profile.enterprise.verification.EnterpriseVerificationEvidenceCodec;
import org.dromara.profile.enterprise.verification.EnterpriseVerificationProviderProperties;
import org.dromara.profile.enterprise.verification.EnterpriseVerificationProviderRegistry;
import org.dromara.profile.enterprise.verification.EnterpriseVerificationSecurityAuditRecorder;
import org.dromara.profile.enterprise.verification.MybatisEnterpriseVerificationAttemptRepository;
...
class EnterpriseApplicationMySqlE2ETest {
    ...
    private void finish(EnterpriseApplicationService service, long applicationId, int snapshotVersion) {
    ...
    private Fixture fixture(SqlSession session) {
        ...
        MybatisEnterpriseApplicationRepository applications = new MybatisEnterpriseApplicationRepository(
            session.getMapper(EnterpriseApplicationMapper.class), json);
        MybatisEnterpriseVerificationAttemptRepository attemptsRepository =
            new MybatisEnterpriseVerificationAttemptRepository(
                session.getMapper(EnterpriseVerificationAttemptMapper.class),
                new EnterpriseVerificationEvidenceCodec(json));
        ...
        EnterpriseVerificationAttemptCoordinator attempts = new EnterpriseVerificationAttemptCoordinator(
            providers, attemptsRepository, new EnterpriseVerificationSecurityAuditRecorder(attemptsRepository));
        ...
        EnterpriseApplicationService service = new EnterpriseApplicationService(
            applications, materials, providers, attempts, workflow, config, Clock.fixed(NOW, ZoneOffset.UTC));
    ...
    private record Fixture(SqlSession session, EnterpriseApplicationService service,
