package org.dromara.profile.enterprise.verification.mapper;
...
public interface EnterpriseVerificationAttemptMapper {

    @Select("""
        select a.enterprise_application_id as application_id,
               s.enterprise_submission_id as submission_id,
               a.provider_code,
               a.status
          from profile_enterprise_application a
          join profile_enterprise_submission s
            on s.enterprise_application_id = a.enterprise_application_id
           and s.submission_seq = a.submission_seq
           and s.del_flag = '0'
         where a.enterprise_application_id = #{applicationId}
           and a.del_flag = '0'
         for update
        """)
    ...
    @Insert("""
        insert into profile_verification_attempt (
            ...
            #{verificationAttemptId}, 'ENTERPRISE', #{applicationId}, #{submissionId},
    ...
    @Insert("""
        insert into profile_operation_audit (
            ...
            #{auditId}, 'ENTERPRISE', #{applicationId}, 'PROVIDER_CALLBACK',
            0, 'profile:enterprise:providerCallback', #{result}, #{failureCategory}, #{occurredTime},
