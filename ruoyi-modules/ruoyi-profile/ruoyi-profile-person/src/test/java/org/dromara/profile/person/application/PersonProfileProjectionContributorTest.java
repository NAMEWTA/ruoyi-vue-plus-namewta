...
class PersonProfileProjectionContributorTest {
    ...
    void returnsOnlyRepositoryApprovedActiveProjection() {
        PersonApplicationRepository repository = mock(PersonApplicationRepository.class);
        when(repository.findActiveProjections(Set.of(101L, 102L))).thenReturn(List.of(
            new PersonActiveProjection(101L, 9001L, Instant.parse("2026-09-01T12:00:00Z"))));

        PersonProfileProjectionContributor contributor = new PersonProfileProjectionContributor(repository);
