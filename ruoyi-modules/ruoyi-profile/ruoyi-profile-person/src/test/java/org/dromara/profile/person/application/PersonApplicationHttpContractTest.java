...
class PersonApplicationHttpContractTest {
    ...
    void exposesCurrentUserOnlyRoutesAndNeverLogsIdentityBodies() {
        ...
        assertThat(PersonDraftCommand.class.getRecordComponents())
    ...
    void separatesReadSaveAndSubmitCommands() throws Exception {
        assertThat(PersonApplicationController.class.getMethod("current").getAnnotation(GetMapping.class))
            .isNotNull();
        assertThat(PersonApplicationController.class.getMethod("save", PersonDraftCommand.class)
            .getAnnotation(PostMapping.class).value()).isEmpty();
        assertThat(PersonApplicationController.class.getMethod("submit", PersonSubmitCommand.class)
            .getAnnotation(PostMapping.class).value()).containsExactly("/submit");
        assertThat(PersonApplicationService.class.getMethod("save", long.class, PersonDraftCommand.class)
            .isAnnotationPresent(DSTransactional.class)).isTrue();
        assertThat(PersonApplicationService.class.getMethod("submit", long.class, int.class)
            .isAnnotationPresent(DSTransactional.class)).isTrue();
        assertThat(PersonApplicationService.class.getMethod("handleProcessEvent",
