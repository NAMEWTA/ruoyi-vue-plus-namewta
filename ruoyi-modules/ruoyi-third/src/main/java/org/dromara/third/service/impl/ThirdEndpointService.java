...
public class ThirdEndpointService implements ThirdEndpointUseCase {
    ...
    public List<ThirdEndpointVo> list(Long providerId, String keyword) {
        ...
        if (keyword != null && !keyword.isBlank()) query.and(q -> q.like(ThirdEndpoint::getEndpointCode, keyword.trim()).or().like(ThirdEndpoint::getEndpointName, keyword.trim()));
        return endpointMapper.selectList(query).stream().map(ThirdEndpointService::toVo).toList();
    }
    ...
    public void save(ThirdEndpointBo bo) {
        ThirdProvider provider = providerMapper.selectOne(new LambdaQueryWrapper<ThirdProvider>().eq(ThirdProvider::getProviderId, bo.getProviderId()).eq(ThirdProvider::getDelFlag, "0"));
        ...
        long duplicate = endpointMapper.selectCount(new LambdaQueryWrapper<ThirdEndpoint>().eq(ThirdEndpoint::getProviderCode, bo.getProviderCode()).eq(ThirdEndpoint::getEndpointCode, bo.getEndpointCode()).eq(ThirdEndpoint::getDelFlag, "0").ne(bo.getEndpointId() != null, ThirdEndpoint::getEndpointId, bo.getEndpointId()));
        if (duplicate > 0) throw new ServiceException("Endpoint code already exists for provider");
        ...
        entity.setProviderId(provider.getProviderId()); entity.setProviderCode(provider.getProviderCode()); entity.setEndpointCode(bo.getEndpointCode().trim()); entity.setEndpointName(bo.getEndpointName().trim());
        entity.setHttpMethod(ThirdEndpointSecurity.validateMethod(bo.getHttpMethod())); entity.setRelativePath(ThirdEndpointSecurity.validateRelativePath(bo.getRelativePath()));
        entity.setRequestMode(ThirdEndpointSecurity.validateRequestMode(bo.getRequestMode())); entity.setResponseMode(ThirdEndpointSecurity.validateResponseMode(bo.getResponseMode()));
        entity.setPathSchemaJson(bo.getPathSchemaJson()); entity.setQuerySchemaJson(bo.getQuerySchemaJson()); entity.setHeaderSchemaJson(bo.getHeaderSchemaJson()); entity.setBodySchemaJson(bo.getBodySchemaJson()); entity.setResponseSchemaJson(bo.getResponseSchemaJson()); entity.setOverrideJson(bo.getOverrideJson()); entity.setStatus(bo.getStatus()); entity.setIdempotent(bo.getIdempotent());
    ...
    private static ThirdEndpointVo toVo(ThirdEndpoint x) { return new ThirdEndpointVo(x.getEndpointId(), x.getProviderId(), x.getProviderCode(), x.getEndpointCode(), x.getEndpointName(), x.getHttpMethod(), x.getRelativePath(), x.getRequestMode(), x.getResponseMode(), x.getPathSchemaJson(), x.getQuerySchemaJson(), x.getHeaderSchemaJson(), x.getBodySchemaJson(), x.getResponseSchemaJson(), x.getOverrideJson(), x.getStatus(), x.getIdempotent(), x.getRateLimit(), x.getConcurrencyLimit(), x.getRetryCount(), x.getSensitiveFieldsJson(), x.getAdapterCode(), x.getVersion(), x.getCreateTime(), x.getUpdateTime()); }
}
