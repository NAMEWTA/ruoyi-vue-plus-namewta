...
本索引覆盖 ruoyi-modules/ruoyi-demo，以本目录的 pom.xml 和源码为准。

## Purpose

说明 ruoyi-demo 的职责边界、组成和最小验证入口。
...
在后端仓库根目录执行 `mvn -pl ruoyi-modules/ruoyi-demo -am -DskipTests compile`，再按模块测试配置执行验证。
